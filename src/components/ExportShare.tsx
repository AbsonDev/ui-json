...
export default function ${appName.replace(/\s+/g, '')}App() {
  ...
  const renderComponent = (component) => {
    switch (component.type) {
      case 'text':
        return <Text key={component.id} style={{ fontSize: component.fontSize || 16, color: component.color }}>{component.content}</Text>;
      case 'button':
        return <Button key={component.id} title={component.text} onPress={() => console.log(component.action)} />;
      case 'input':
        return <Text key={component.id}>{component.label || component.placeholder}</Text>;
      case 'image':
        return <Text key={component.id}>Image: {component.source}</Text>;
      default:
        return <Text key={component.id}>Component: {component.type}</Text>;
    }
  };
...
<body>
  ...
  <script>
    ...
    function renderComponent(component) {
      switch (component.type) {
        case 'text':
          return \`<p style="font-size: \${component.fontSize || 16}px; color: \${component.color || '#000'}">\${component.content}</p>\`;
        case 'button':
          return \`<button style="padding: 10px 20px; margin: 5px;">\${component.text}</button>\`;
        case 'input':
          return \`<input type="text" placeholder="\${component.placeholder || ''}" style="padding: 8px; margin: 5px; width: 100%; box-sizing: border-box;" />\`;
        case 'image':
          return \`<img src="\${component.source}" style="max-width: 100%;" alt="Image" />\`;
        default:
    ...
    const screen = appData.screens[appData.initialScreen];
    const root = document.getElementById('app-root');
    if (screen && screen.components) {
      root.innerHTML = '<h2>' + screen.title + '</h2>' + screen.components.map(renderComponent).join('');
    }
  </script>
</body>
</html>
